...
Stack: detecte o framework de IA/LLM e o vector store já presentes no codebase (ex.: LangChain, LlamaIndex, Spring AI, SDK oficial do provedor, pgvector, Pinecone, Qdrant, Redis). Se nenhum estiver presente, proponha a opção mais simples que atenda ao requisito (ex.: chamada direta ao SDK do provedor de LLM + busca tradicional, sem framework de orquestração).
...
Sempre preferir a alternativa de menor custo computacional e operacional antes de avançar para IA mais sofisticada.
...
* Frameworks de orquestração de agentes (ex.: LangGraph, LangChain agents): somente para fluxos complexos com múltiplas decisões
...
* Não introduza frameworks de orquestração (LangGraph, LangChain ou similares) sem justificativa concreta
...
* Não usar modelos, vetores ou agentes se a solução simples resolver com qualidade suficiente
